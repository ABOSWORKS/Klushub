...
- 2026-03-25: Added Playwright QA setup in Codex workspace (`package.json`, `playwright.config.js`, smoke tests in `tests/smoke`). Installed `@playwright/test` + Chromium. `npm run test:smoke` now passes (`SMOKE_OK`) via a runner-free Node smoke script designed for this sandbox.
